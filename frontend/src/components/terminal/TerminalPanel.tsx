...
interface TerminalPanelProps {
  ...
  accent?: string; // tailwind color class like "text-emerald-400"
...
export function TerminalPanel({ title, icon, accent = "text-sky-400", children, actions, compact = false, className = "" }: TerminalPanelProps) {
  return (
    <div className={`h-full flex flex-col rounded-2xl border border-border/60 bg-card/80 shadow-[0_18px_48px_-38px_rgba(15,23,42,0.45)] overflow-hidden ${className}`}>
      <div className="flex items-center justify-between gap-2 border-b border-border/40 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <GripVertical className="h-3.5 w-3.5 text-muted-foreground/40 cursor-grab active:cursor-grabbing drag-handle" />
