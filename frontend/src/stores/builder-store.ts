  ...
  persist(
    (set) => ({
      ...
      loadFromStrategy: (record) => {
        // API stores percentages as decimals (0.02 = 2%); builder store uses whole numbers (2 = 2%)
        set({
          ...DEFAULT_STATE,
          name: record.name,
          ...
          stopLoss: (record.stop_loss_pct ?? 0.02) * 100,
          takeProfit: (record.take_profit_pct ?? 0.05) * 100,
          positionSize: (record.position_size_pct ?? 0.05) * 100,
          timeframe: record.timeframe,
          symbols: record.symbols,
          commissionPct: (record.commission_pct ?? 0.001) * 100,
          slippagePct: (record.slippage_pct ?? 0.0005) * 100,
          trailingStopEnabled: record.trailing_stop_pct != null && record.trailing_stop_pct > 0,
          trailingStop: (record.trailing_stop_pct ?? 0.01) * 100,
          ...
          maxExposurePct: (record.max_exposure_pct ?? 1.0) * 100,
          maxLossPct: (record.max_loss_pct ?? 0) * 100,
